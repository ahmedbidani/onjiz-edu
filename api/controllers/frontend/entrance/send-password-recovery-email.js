module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    var userRecord = await User.findOne({ emailAddress: inputs.emailAddress.toLowerCase() });
    if(!userRecord) userRecord = await Parent.findOne({ emailAddress: inputs.emailAddress.toLowerCase() });
    ...
    if(userRecord.userType) {
      ...
      userRecord = await Parent.update({ id: userRecord.id })
        .set({ password: hashed })
        .fetch();
    }
