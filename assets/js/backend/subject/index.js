...
class ListIndexSubjectBackendEKP {
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashSubject.with({ ids: id, _csrf: $('[name="_csrf"]').val() }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  //END ALERT AND TRASH
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusSubject.with({ id: id, _csrf: $('[name="_csrf"]').val() }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {
