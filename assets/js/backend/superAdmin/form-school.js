class IndexFormSchoolSABackendEKP extends BaseBackendEKP {
  constructor() {
    super( );
    this.initialize();
  }
  ...
  initialize() {
    ...
    this.form = new FormIndexSchoolSABackendEKP(this);
  }
}

class FormIndexSchoolSABackendEKP {
  constructor(opts) {
    _.extend(this, opts);
  ...
  initValidation() {
    ...
    _this.formObj.formValidation({
      ...
      .on('success.form.fv', function (e) {
        ...
        if (manner === 'edit') {
          Cloud.editSchool.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
          ...
          Cloud.addSchool.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              if (err.responseInfo.statusCode == 400) {
                _this.alert.removeClass('hidden alert-success').addClass("alert-danger").html(responseBody.message);
                return; 
              ...
              window.location.replace('/sa/school-' + responseBody.id + '/admin/add');
              return;
            }
          });
        }
