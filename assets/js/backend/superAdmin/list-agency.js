class IndexListAgencySABackendEKP extends BaseBackendEKP {
    constructor() {
      super( );
      this.initialize();
    }
    ...
    initialize() {
      ...
      this.list = new ListIndexAgencySABackendEKP(this); 
    }
  }
  
  
  class ListIndexAgencySABackendEKP {
    constructor(opts) {
      _.extend(this, opts);
  
      this.langUrl = this.lang == 'en' ? '' : '/js/backend/datatable.json';
      this.tblId = 'tblAgency';
    ...
    initSweetAlert(id) {
      swal({
        ...
        if (value) {
          ...
          //get AJAX data
          Cloud.trashAgency.with({ ids: id,  _csrf: csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
