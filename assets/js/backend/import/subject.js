class IndexFormImportSubjectBackendEKP extends BaseBackendEKP {
    constructor() {
      super( );
      this.initialize();
    }
    ...
    initialize() {
      ...
      this.form = new FormIndexImportSubjectBackendEKP(this);
    }
  }
  
  class FormIndexImportSubjectBackendEKP {
    constructor(opts) {
      _.extend(this, opts);
      this.formId = 'formImportSubject';
      ...
      if (inputFiles.length) {
        ...
        inputFiles.each((i, input) => {
          $(input).on('change', (e) => {
            ...
            Cloud.importSubjectExcel.with(_data).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
              if (err) {
                console.log(err);
                let title = _this.messages.cannotImport;
                if (err.responseInfo.body.code == 'SUBJECT_ERROR_IMPORT') title = _this.messages.error + ', ' + err.responseInfo.body.message;
                swal({
                  title: title,
                  icon: 'error',
                  button: {
