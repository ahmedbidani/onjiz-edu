class IndexFormImportScheduleBackendEKP extends BaseBackendEKP {
    constructor() {
      super( );
      this.initialize();
    }
    ...
    initialize() {
      ...
      this.form = new FormIndexImportScheduleBackendEKP(this);
    }
  }
  
  class FormIndexImportScheduleBackendEKP {
    constructor(opts) {
      _.extend(this, opts);
      this.formId = 'formImportSchedule';
      ...
      if (inputFiles.length) {
        ...
        inputFiles.each((i, input) => {
          $(input).on('change', (e) => {
            ...
            Cloud.importScheduleExcel.with(_data).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
              if (err) {
                console.log(err);
                let title = _this.messages.cannotImport;
                if (err.responseInfo.body.code == 'SCHEDULE_ERROR_IMPORT') title = _this.messages.error + ', ' + err.responseInfo.body.message;
                swal({
                  title: title,
                  icon: 'error',
                  button: {
