class IndexFormImportFoodBackendEKP extends BaseBackendEKP {
    constructor() {
      super( );
      this.initialize();
    }
    ...
    initialize() {
      ...
      this.form = new FormIndexImportFoodBackendEKP(this);
    }
  }
  
  class FormIndexImportFoodBackendEKP {
    constructor(opts) {
      _.extend(this, opts);
      this.formId = 'formImportFood';
      ...
      if (inputFiles.length) {
        ...
        inputFiles.each((i, input) => {
          $(input).on('change', (e) => {
            ...
            Cloud.importFoodExcel.with(_data).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
              if (err) {
                console.log(err);
                let title = _this.messages.cannotImport;
                if (err.responseInfo.body.code == 'FOOD_ERROR_IMPORT') title = _this.messages.error + ', ' + err.responseInfo.body.message;
                swal({
                  title: title,
                  icon: 'error',
                  button: {
