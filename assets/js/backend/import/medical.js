class IndexFormImportMedicalBackendEKP extends BaseBackendEKP {
  constructor() {
    super( );
    this.initialize();
  }
  ...
  initialize() {
    ...
    this.form = new FormIndexImportMedicalBackendEKP(this);
  }
}

class FormIndexImportMedicalBackendEKP {
  constructor(opts) {
    _.extend(this, opts);
    this.formId = 'formImportMedical';
  ...
  initUploadFile() {
    ...
    let medicalId = filterParams.get('id') || null;
    ...
    if (inputFiles.length) {
      ...
      inputFiles.each((i, input) => {
        $(input).on('change', (e) => {
          ...
          Cloud.importMedicalExcel.with(_data).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              let title = _this.messages.cannotImport;
              if (err.responseInfo.body.code == 'MEDICAL_ERROR_IMPORT') title = _this.messages.error + ', ' + err.responseInfo.body.message;
              swal({
                title: title,
                icon: 'error',
                button: {
