class IndexListSchoolAGBackendEKP extends BaseBackendEKP {
    constructor() {
      super( );
      this.initialize();
    }
    ...
    initialize() {
      ...
      this.list = new ListIndexSchoolAGBackendEKP(this); 
    }
  }
  
  
  class ListIndexSchoolAGBackendEKP {
    constructor(opts) {
      _.extend(this, opts);
    ...
    initDataTable() {
      let _this = this;
      let params = {};
      params.agencyID = _this.tableObj.attr('data-Agency');
      //cloud success  
      var table = this.tableObj.DataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "/api/v1/agency/school/search?agencyID=" + params.agencyID,
        ...
        columnDefs: [
          { "orderable": false, "targets": [0, 3, 4, -1] }
        ],
        "order": [[1, "asc"]],
        "iDisplayLength": 50,
    ...
    initSweetAlert(id) {
      swal({
        ...
        if (value) {
          ...
          //get AJAX data
          Cloud.deleteSchoolAG.with({ ids: id,  _csrf: csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
