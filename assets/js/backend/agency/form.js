class IndexFormSchoolAGBackendEKP extends BaseBackendEKP {
  constructor() {
    super( );
    this.initialize();
  }
  ...
  initialize() {
    ...
    this.form = new FormIndexSchoolAGBackendEKP(this);
  }
}

class FormIndexSchoolAGBackendEKP {
  constructor(opts) {
    _.extend(this, opts);
    this.formId = 'formSchool';
  ...
  initValidation() {
    ...
    _this.formObj.formValidation({
      ...
      .on('success.form.fv', function (e) {
        ...
        if (_this.schoolPhoto) {
          tmpData['photo'] = _this.schoolPhoto;
        }
        //reset form validator
        let manner = _this.formObj.attr('data-manner');
        if (manner === 'edit') {
          Cloud.editSchoolAG.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
          ...
          Cloud.addSchoolAG.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              if (err.responseInfo.statusCode == 400) {
                _this.alert.removeClass('hidden alert-success').addClass("alert-danger").html(responseBody.message);
                return; 
              ...
              window.location.replace('/agency/school-' + responseBody.id + '/admin/add');
              return;
            }
          });
        }
  ...
  initUploadFile() {
		...
		if (inputFiles.length) {
			if (_this.schoolPhoto == undefined) {
				_this.schoolPhoto = "";
			}
            ...
            let manner = _this.formObj.attr('data-manner');
            if (manner == 'edit') _data.school = $('#schoolId').val();
            
						Cloud.uploadSchoolPhoto.with(_data).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
							if (err) {
                if (err.responseInfo && err.responseInfo.data && err.responseInfo.data.raw == "E_EXCEEDS_UPLOAD_LIMIT" || err.responseInfo && err.responseInfo.statusCode == 500) {
                  swal({
                    ...
                    window.location.reload();
							...
							//save thumb data to object schoolPhoto
							console.log(responseBody);
							_this.schoolPhoto = responseBody;
